...
library Parser {
    ...
    function parseOrderBytes(
        bytes memory packedEncodedOrder,
        bool noSig
    ) public pure returns (Structs.Order memory, bytes memory) {
        address account;
        uint8 subAccountId;
        uint32 productId;
        uint8 isBuy;
    ...
    function parseApprovedSignerBytes(
        bytes memory packedEncodedApprovedSigner
    ) public pure returns (Structs.ApproveSigner memory, bytes memory) {
        address account;
        uint8 subAccountId;
    ...
    function parseDepositBytes(
        bytes memory packedEncodedDeposit
    ) public pure returns (Structs.Deposit memory, bytes memory) {
        address account;
        uint8 subAccountId;
        address asset;
        uint256 quantity;
        ...
        return (
            Structs.Deposit(account, subAccountId, asset, quantity, nonce),
            sig
        );
    }
    ...
    function parseWithdrawBytes(
        bytes memory packedEncodedWithdraw
    ) public pure returns (Structs.Withdraw memory, bytes memory) {
        address account;
        uint8 subAccountId;
        address asset;
        uint128 quantity;
        ...
        return (
            Structs.Withdraw(account, subAccountId, asset, quantity, nonce),
            sig
        );
    }
    ...
    function parseLiquidateBytes(
        bytes memory packedEncodedLiquidate
    )
        public
        pure
